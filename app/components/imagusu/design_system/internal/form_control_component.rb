  ...
  module DesignSystem
    module Internal
      class FormControlComponent < Imagusu::DesignSystem::BaseComponent
        ...
        def data_with_state(attributes)
          data = attributes.fetch(:data, {})
          raise ArgumentError, "data must be a Hash" unless data.is_a?(Hash)

          normalized = data.to_h do |key, value|
            raise ArgumentError, "data keys must be strings or symbols" unless key.respond_to?(:to_sym)

            [key.to_sym, value]
          end
          raise ArgumentError, "component owns data attributes: state" if normalized.key?(:state)

          normalized.merge(state: state_tokens)
        end

        # Matches the value suffix generated by Rails collection helpers.
        def sanitized_collection_value(value)
